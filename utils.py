...
def get_threshold(predict, label):
    ...
    hit = sum([((label_f[i] == result_f[i]) & (label_f[i] == 1)) for i in range(count*1)])
...
def read_data_P(char=False):
    if not char:
        train_sentence = np.load('data/train_sentence_p.npy')
        label = np.load('data/train_label_p.npy')
        dev_sentence = np.load('data/dev_sentence_p.npy')
        dev_label = np.load('data/dev_label_p.npy')
        train_postag = np.load('data/train_postag_p.npy')
        dev_postag = np.load('data/dev_postag_p.npy')
        train_ner = np.load('data/train_ner_p.npy')
        dev_ner = np.load('data/dev_ner_p.npy')
    else:
        train_sentence = np.load('data/train_sentence_char.npy')
        label = np.load('data/train_label_char.npy')
        dev_sentence = np.load('data/dev_sentence_char.npy')
